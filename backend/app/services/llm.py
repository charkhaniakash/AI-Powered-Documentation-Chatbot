...
class LLMService:
        ...
        try:
            ...
            response = self.client.chat.completions.create(
                ...
                messages=[
                    {
                        "role": "system",
                        "content": "You are a skilled summarizer. Create clear, concise summaries."
                    },
                    {
                        "role": "user",
                        "content": prompt
            ...
            return combined_text[:max_length * 5] + "..."
    ...
    def extract_keywords(self, query: str, max_keywords: int = 5) -> List[str]:
        """
        Extract keywords from query for enhanced search.
        ...
        prompt = f"""Extract the {max_keywords} most important keywords from this query.
        ...
        try:
            response = self.client.chat.completions.create(
                ...
                messages=[
                    {
                        "role": "system",
                        "content": "You extract key terms for search."
                    },
                    {
                        "role": "user",
                        "content": prompt
    ...
    def generate_follow_up_questions(
        ...
        Helps users explore the topic further.
        Improves engagement and discoverability.
        ...
        prompt = f"""Based on this Q&A, generate {num_questions} relevant follow-up questions.
            ...
            2. Dig deeper into the answer
            3. Are likely to be answerable from the same documents
        ...
        try:
            response = self.client.chat.completions.create(
                ...
                messages=[
                    {
                        "role": "system",
                        "content": "You generate insightful follow-up questions."
                    },
                    {
                        "role": "user",
                        "content": prompt
